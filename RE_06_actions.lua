...
local function collect_dominant_lock_specs(action)
    if action == nil then
        return nil
    end

    if type(action.getDominantLockSpec) == "function" then
        local specs = action:getDominantLockSpec()
        if type(specs) == "table" and #specs > 0 then
            return specs
        end
    end

    if type(action.getDominantLockSpecs) == "function" then
        local specs = action:getDominantLockSpecs()
        if type(specs) == "table" and #specs > 0 then
            return specs
        end
    end
...
function GuardedAction:getDominantLockSpec()
    return collect_dominant_lock_specs(self._action)
end
...
function MultiAction:getDominantLockSpec()
    local collected = {}
    for _, action in ipairs(self._actions) do
        local specs = collect_dominant_lock_specs(action)
        if type(specs) == "table" then
            for _, spec in ipairs(specs) do
                collected[#collected + 1] = spec
            end
        end
    end
    if #collected == 0 then
...
function PersistentAction.new(action, duration, dominant_lock_enabled)
    local instance = Action.new()
    instance._action = action
    instance._duration = duration or 0.0
    instance._startTime = 0.0
    instance._isRunning = false
    instance._dominantLockEnabled = tonumber(dominant_lock_enabled) == 1
    instance._dominantLockHandle = nil
...
function PersistentAction:enter(current_time, _)
    if not self._isRunning then
        ...
        if self._action ~= nil then
            enter_result = self._action:enter(current_time, false)
        end
        if self._dominantLockEnabled and enter_result ~= false then
            local dominant_specs = collect_dominant_lock_specs(self._action)
            if type(dominant_specs) == "table" and #dominant_specs > 0 then
                self._dominantLockHandle = input.acquireDominantLock(dominant_specs)
            end
        end
        active_actions[#active_actions + 1] = self
    end
...
function PersistentAction:stop()
    if self._isRunning then
        if self._dominantLockHandle ~= nil then
            input.releaseDominantLock(self._dominantLockHandle)
            self._dominantLockHandle = nil
        end
        if self._action ~= nil then
            self._action:leave()
        end
...
local RefcountAction = {}
RefcountAction.__index = RefcountAction
setmetatable(RefcountAction, { __index = Action })

function RefcountAction.new(inner_action)
    return setmetatable({
        haptics = inner_action and inner_action.haptics or nil,
        _inner = inner_action,
        _refs = 0,
    }, RefcountAction)
end

function RefcountAction:getCurrentHaptics()
    if self._inner ~= nil and type(self._inner.getCurrentHaptics) == "function" then
        return self._inner:getCurrentHaptics()
    end
    return self.haptics
end

function RefcountAction:getDominantLockSpec()
    if self._inner ~= nil and type(self._inner.getDominantLockSpec) == "function" then
        return self._inner:getDominantLockSpec()
    end
    return nil
end

function RefcountAction:enter(current_time, from_voice_recognition)
    self._refs = self._refs + 1
    if self._refs == 1 and self._inner ~= nil then
        return self._inner:enter(current_time, from_voice_recognition)
    end
end

function RefcountAction:update(current_time)
    if self._refs > 0 and self._inner ~= nil and type(self._inner.update) == "function" then
        self._inner:update(current_time)
    end
end

function RefcountAction:leave()
    self._refs = self._refs - 1
    if self._refs <= 0 then
        self._refs = 0
        if self._inner ~= nil and type(self._inner.leave) == "function" then
            self._inner:leave()
        end
    end
end

function RefcountAction:reset()
    self._refs = 0
    if self._inner ~= nil and type(self._inner.reset) == "function" then
        self._inner:reset()
    end
end
...
actions.RefcountAction = RefcountAction
actions.input = input
