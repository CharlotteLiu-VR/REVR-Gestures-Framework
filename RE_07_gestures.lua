...
local function validation_pressed(value, lower_threshold)
    return (value or 0.0) > lower_threshold
end
...
function LocationBasedGesture.new(lower_threshold, upper_threshold, offset)
    ...
    instance._inZone = false
    instance._validationGripPrev = false
    instance._validationTriggerPrev = false
    instance._gripValidateOnRelease = false
    instance._triggerValidateOnRelease = false
    return setmetatable(instance, LocationBasedGesture)
end

function LocationBasedGesture:reset()
    Gesture.reset(self)
    self._inZone = false
    self._validationGripPrev = false
    self._validationTriggerPrev = false
    self._gripValidateOnRelease = false
    self._triggerValidateOnRelease = false
end

function LocationBasedGesture:_syncLocationZone(value, validation)
    if not self._inZone then
        if value < self.lowerThreshold then
            self._inZone = true
            self._gripValidateOnRelease = validation_pressed(validation.grip, self.gripLowerThreshold)
            self._triggerValidateOnRelease = validation_pressed(validation.trigger, self.triggerLowerThreshold)
        end
    elseif value > self.upperThreshold then
        self._inZone = false
        self._gripValidateOnRelease = false
        self._triggerValidateOnRelease = false
    end
...
function LocationBasedGesture:_updateCore(current_time, value, validation, current_haptics)
    self:_syncLocationZone(value, validation)
    Gesture._updateCore(self, current_time, value, validation, current_haptics)
end

function LocationBasedGesture:_updateTriggerGesture(current_time, value, validation, current_haptics)
    local action_haptics = get_current_haptics(self.triggerAction)
    if action_haptics ~= nil then
        current_haptics = action_haptics
    end

    local trigger_pressed = validation_pressed(validation.trigger, self.triggerLowerThreshold)
    local trigger_press_edge = trigger_pressed and not self._validationTriggerPrev
    local trigger_release_edge = not trigger_pressed and self._validationTriggerPrev
    local trigger_validation_edge = self._triggerValidateOnRelease
        and trigger_release_edge
        or (not self._triggerValidateOnRelease and trigger_press_edge)
    ...
    elseif (current_time - self._lastTriggerTime) > self.coolDown
        and (not validation.triggerUsed)
        and self._inZone
        and trigger_validation_edge then
        local enter_result = safe_call(self.triggerAction, "enter", current_time, false)
        if entered_action(enter_result) then
            self:playHaptics(current_time, current_haptics.enter, current_haptics.touchEnter, "enter")
            self.inTriggerGesture = true
    ...
    self._validationTriggerPrev = trigger_pressed
end

function LocationBasedGesture:_updateGripGesture(current_time, value, validation, current_haptics)
    local action_haptics = get_current_haptics(self.gripAction)
    if action_haptics ~= nil then
        current_haptics = action_haptics
    end

    local grip_pressed = validation_pressed(validation.grip, self.gripLowerThreshold)
    local grip_press_edge = grip_pressed and not self._validationGripPrev
    local grip_release_edge = not grip_pressed and self._validationGripPrev
    local grip_validation_edge = self._gripValidateOnRelease
        and grip_release_edge
        or (not self._gripValidateOnRelease and grip_press_edge)
    ...
    self._validationGripPrev = grip_pressed
end
...
-- 同手 LocationBasedGesture 处于 validating 区域（distance² < lowerThreshold）时，
-- 抑制 use*Up / use*Down，避免槽位抓握与交互 F 等抢同一侧抓握；不做手柄面点积判定。
local USE_GESTURE_SUPPRESS_VALUE = 0.0

local function is_hand_in_location_validating_zone(gesture_list, head_pose, controller_pose)
    if type(gesture_list) ~= "table" or head_pose == nil or controller_pose == nil then
        return false
    end

    for _, gesture in ipairs(gesture_list) do
        if gesture.enabled and gesture.offset ~= nil then
            local distance_squared = numerics.location_distance_squared(
                head_pose,
                gesture.offset,
                controller_pose
            )
            if distance_squared < gesture.lowerThreshold then
                return true
            end
        end
    end
...
function GestureTracker.new(inventory, context)
    ...
    register_gesture(tracker, "chestLeft", LocationBasedGesture.new(0.02, 0.03, numerics.new_vector(-0.1, -0.4, 0.0)))
    register_gesture(tracker, "chestRight", LocationBasedGesture.new(0.02, 0.03, numerics.new_vector(-0.1, -0.4, 0.0)))
    ...
    register_gesture(tracker, "shoulderInventoryLeft", LocationBasedGesture.new(0.05, 0.1, numerics.new_vector(-0.25, 0.0, 0.1)))
    register_gesture(tracker, "shoulderInventoryRight", LocationBasedGesture.new(0.05, 0.1, numerics.new_vector(0.3, 0.0, 0.0)))
    register_gesture(tracker, "shoulderWeaponLeft", LocationBasedGesture.new(0.05, 0.1, numerics.new_vector(-0.3, 0.0, 0.0)))
    register_gesture(tracker, "shoulderWeaponRight", LocationBasedGesture.new(0.02, 0.03, numerics.new_vector(0.2, 0.0, 0.15)))
    register_gesture(tracker, "triggerLeft", Gesture.new(-0.6, -0.4))
    register_gesture(tracker, "triggerRight", Gesture.new(-0.6, -0.4))
    register_gesture(tracker, "upperAreaLeft", Gesture.new(-0.1, -0.05))
    register_gesture(tracker, "upperAreaRight", Gesture.new(-0.1, -0.05))
    ...
    register_gesture(tracker, "swipeLeftHandLeft", Gesture.new(-1.0, -0.8))
    register_gesture(tracker, "swipeLeftHandRight", Gesture.new(-1.0, -0.8))
    register_gesture(tracker, "swipeLeftHandUp", Gesture.new(-1.0, -0.8))
    register_gesture(tracker, "swipeLeftHandDown", Gesture.new(-1.0, -0.8))
    register_gesture(tracker, "swipeRightHandLeft", Gesture.new(-2.0, -1.0))
    register_gesture(tracker, "swipeRightHandRight", Gesture.new(-2.0, -1.0))
    register_gesture(tracker, "swipeRightHandUp", Gesture.new(-2.0, -1.0))
    register_gesture(tracker, "swipeRightHandDown", Gesture.new(-2.0, -1.0))
    ...
    register_gesture(tracker, "thrustLeft", Gesture.new(-10.0, -8.0))
    register_gesture(tracker, "retractLeft", Gesture.new(-12.0, -10.0))
    register_gesture(tracker, "thrustRight", Gesture.new(-10.0, -8.0))
    register_gesture(tracker, "retractRight", Gesture.new(-12.0, -10.0))
    register_gesture(tracker, "pullPin", Gesture.new(-2.0, -1.0))
...
function GestureTracker:update(current_time, delta_time, vr_state)
    ...
    local thrust_retract_purity = 2.0
    ...
    local head_pose = vr_state.headPose
    local left_use_blocked = is_hand_in_location_validating_zone(
        self._locationBasedGesturesLeft,
        head_pose,
        vr_state.leftTouchPose
    )
    local left_up_value = left_use_blocked and USE_GESTURE_SUPPRESS_VALUE or (vr_state.leftTouchPose.left.y or 0.0)
    local left_down_value = left_use_blocked and USE_GESTURE_SUPPRESS_VALUE or (-(vr_state.leftTouchPose.left.y or 0.0))
    update_if_enabled(self.useLeftUp, current_time, left_up_value, left_validation)
    update_if_enabled(self.useLeftDown, current_time, left_down_value, left_validation)
    ...
    local right_use_blocked = is_hand_in_location_validating_zone(
        self._locationBasedGesturesRight,
        head_pose,
        vr_state.rightTouchPose
    )
    local right_up_value = right_use_blocked and USE_GESTURE_SUPPRESS_VALUE or (-(vr_state.rightTouchPose.left.y or 0.0))
    local right_down_value = right_use_blocked and USE_GESTURE_SUPPRESS_VALUE or (vr_state.rightTouchPose.left.y or 0.0)
    update_if_enabled(self.useRightUp, current_time, right_up_value, right_validation)
    update_if_enabled(self.useRightDown, current_time, right_down_value, right_validation)
