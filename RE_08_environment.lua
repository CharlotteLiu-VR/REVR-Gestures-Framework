...
local controller_indices_buffer = { nil, nil }

local function clear_controller_indices_buffer()
    controller_indices_buffer[1] = nil
    controller_indices_buffer[2] = nil
end
...
local function normalize_controller_indices(controller_indices)
    ...
    local ok_iter, extracted = pcall(function()
        clear_controller_indices_buffer()
        for index, value in ipairs(controller_indices) do
            controller_indices_buffer[index] = value
            if index >= 2 then
                break
            end
        end
        return controller_indices_buffer
    end)
...
function environment:updateTime(current_time, delta_time)
    ...
    if delta == nil then
        if self.time._last_clock ~= nil then
...
function environment:getEnhancer()
...
function environment:publishEnhancer(enhancer)
    if type(enhancer) ~= "table" then
        return nil
    end
    _G.VR_ENHANCER = enhancer
    return enhancer
end

function environment:installHapticFeedbackManager(enhancer)
    local ok, hapticFeedbackModule = pcall(require, "RE_10_HapticFeedback")
    if ok and type(hapticFeedbackModule) == "table" and type(hapticFeedbackModule.ensureInstalled) == "function" then
        hapticFeedbackModule.ensureInstalled(enhancer)
    end
end

function environment:attachEnhancerRuntime(enhancer)
    if type(enhancer) ~= "table" then
        return false
    end

    self:publishEnhancer(enhancer)
    self:installHapticFeedbackManager(enhancer)
    if enhancer._runtimeHooksInstalled or rawget(_G, "VR_RUNTIME_HOOKS_INSTALLED") then
        enhancer._runtimeHooksInstalled = true
        return true
    end

    local function tick_from_update_behavior()
        enhancer:tick("UpdateBehavior")
    end

    if re and re.on_pre_application_entry then
        re.on_pre_application_entry("UpdateBehavior", tick_from_update_behavior)
    end
    ...
    if re and re.on_frame then
        re.on_frame(function()
            ...
            if enhancer._fallback_active then
                if enhancer._last_tick_source == "UpdateBehavior" and (now - enhancer._last_tick_at) <= 0.2 then
                    enhancer._fallback_active = false
                    return
                end

                enhancer:tick("on_frame")
                return
            end

            -- UpdateBehavior may stall on some builds; keep on_frame fallback alive.
            if enhancer._last_tick_at == 0.0 or (now - enhancer._last_tick_at) > 0.1 then
                enhancer._fallback_active = true
                enhancer:tick("on_frame")
            end
        end)
    end
    ...
    enhancer._runtimeHooksInstalled = true
    _G.VR_RUNTIME_HOOKS_INSTALLED = true
    return true
end

function environment:setHapticFeedbackEnabled(enabled)
    self.hapticFeedbackEnabled = enabled ~= false
    if self.gestureSets ~= nil and type(self.gestureSets._context) == "table" then
        self.gestureSets._context.haptic_player = self.hapticFeedbackEnabled and self.hapticPlayer or nil
    end
    return self.hapticFeedbackEnabled
end

function environment:createGestureSets(inventory)
    self.gestureSets = self.gestures.GestureSets.new(inventory, {
        haptic_player = self.hapticFeedbackEnabled ~= false and self.hapticPlayer or nil,
...
function environment:initialize(options)
    if self._initialized then
        if options and options.hapticFeedbackEnabled ~= nil then
            self:setHapticFeedbackEnabled(options.hapticFeedbackEnabled)
        end
        return self
    end
    ...
    elseif _G.VRInputBackend ~= nil then
        self:setInputBackend(_G.VRInputBackend)
    end

    if options and options.hapticFeedbackEnabled ~= nil then
        self.hapticFeedbackEnabled = options.hapticFeedbackEnabled ~= false
    elseif rawget(_G, "VR_HAPTIC_FEEDBACK_ENABLED") ~= nil then
        self.hapticFeedbackEnabled = rawget(_G, "VR_HAPTIC_FEEDBACK_ENABLED") ~= false
    else
    ...
    if self.haptics.registerCompanionHapticFeedback ~= nil and self.hapticFeedbackEnabled ~= false then
        self.haptics.registerCompanionHapticFeedback(self.hapticPlayer)
    end
    self.vrToMouse = self.input.VRToMouse.new(self)
    self:createGestureSets(nil)
    self:setHapticFeedbackEnabled(self.hapticFeedbackEnabled)
    self._initialized = true
    _G.VREnvironment = self
    return self
end
